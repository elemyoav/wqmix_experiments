...
'''
A simple n*m grid-world game for N agents trying to capture M box and M' hares. 
...
This version of predator box also includes the observation method get_obs_intersection(agent_ids, global_view=False),
...
but with the difference that the agent and box (ID+1) are given, instead of just their presence (1).
Standard agent observations can be recovered by masking array>0.   
MOVEMENTS
Both predators and box can move to the 4 adjacent states or remain in the current one. Movement is executed 
sequentially: first the predators move in a random order, then the box chooses a random available action 
(i.e. an action that would not lead to a collision with another entity). 
A box is captured if it cannot move (i.e. if 4 agents block all 4 adjacent fields).
REWARDS 
A captured box is removed and yields a collaborative reward of +50. 
Forcing the a box to move (scaring it off), by moving into the same field yields no additional reward. 
Collisions between agents is not punished, and each movement costs additional -0.1 reward. 
An episode ends if all box have been captured.  
OBSERVATIONS
box only react to blocked movements (after the predators have moved), but predator agents observe all positions 
in a square of obs_size=(2*agent_obs+1) centered around the agent's current position. The observations are reshaped 
into a 1d vector of size (2*obs_size), including all predators and box the agent can observe.
...
class StagHunt(MultiAgentEnv):
    ...
    action_labels = {"right" : 0, "down" : 1, "left" : 2, "up" : 3, "stay" : 4,
                      ...
                      "collaborative-push-right" : 9, "collaborative-push-down" : 10, "collaborative-push-left" : 11, "collaborative-push-up" : 12, "sense-box" : 13}
                     
    action_look_to_act = 13
    ...
    def __init__(self, batch_size=None, **kwargs): 
        ...
        if self.state_as_graph:
            ...
            self.box_relational = getattr(args, "box_relational", True)
        ...
        if self.random_ghosts:
            self.ghost_indicator = False        # indicator whether whether box is a ghost (True) or not (False)
            ...
            self.ghost_indicator_pos = [0, 0]   # position of the indicator whether box is a ghost (-1) or not (+1)
        ...
        #self.n_heavy = args.n_heavy
        self.n_heavy = 1
        #self.n_light = args.n_light
        self.n_light = 2
        self.n_boxes = self.n_heavy + self.n_light
        ...
        self.box = np.zeros((self.n_boxes, self.batch_size, 2), dtype=int_type)
        self.box_type = np.ones((self.n_boxes, self.batch_size), dtype=int_type)    # fill with light (1)
        self.box_type[self.n_light:, :] = 2    # set heavy to 2
        self.steps = 0
        self.sum_rewards = 0
        self.reset()
    ...
    def reset(self):
        ...
        # Place n_agents and n_boxess on the grid
        self._place_actors(self.agents, 0)
        # Place the light boxes
        self._place_actors(self.box[:self.n_light, :, :], 1)
        # Place the heavy boxes
        self._place_actors(self.box[self.n_light:, :, :], 2)
    ...
    def step(self, actions):
        ...
        for b in range(self.batch_size):
            for a in np.random.permutation(self.n_agents):
                ...
                self.agents[a, b, :] = self._move_actor(self.agents[a, b, :], actions[a, b], b,self.agent_move_block, 0)
                    
                
        # Move the box
        for b in range(self.batch_size):
            for p in np.random.permutation(self.n_boxes):
                    # Collect all allowed actions for the box
                    possible = []
                    next_to_agent = False
                    # Run through all potential movement actions (without actually moving)
                    for u in range(4):
                        if not self._move_actor(self.box[p, b, :], u, b, np.asarray([0, 1, 2], dtype=int_type))[1]:
                            possible.append(u)
                        if self._move_actor(self.box[p, b, :], u, b, np.asarray([0], dtype=int_type))[1]:
                            next_to_agent = True
                    # Capturing box works differently when the agents have a 'catch' action
                    if self.capture_action:
                        n_catching_agents = 0
                        # box does not move (rest) if they have capture_conditions or less available moves
                        # if len(possible) <= self.capture_conditions[self.box_type[p, b] - 1]:
                        #     possible = []
                        ...
                        for a in range(self.n_agents):
                            if actions[a, b] == self.action_labels['catch'] \
                                ...
                                for u in range(4):
                                    pos = self.box[p, b] + self.actions[u]
                                    if pos[0] == self.agents[a, b, 0] and pos[1] == self.agents[a, b, 1]:
                                        n_catching_agents += 1
                                        break
                        # If the number of neighboring agents that execute 'catch' >= condition, box is captured
                        captured = n_catching_agents >= self.capture_action_conditions[self.box_type[p, b] - 1]
                        #          and len(possible) == 0
                        if n_catching_agents > 0 and not captured:
                            reward[b] += self.miscapture_punishment
                    else:
                        # box is caught when the number of possible moves is less or equal to their capture_condition
                        captured = len(possible) <= self.capture_conditions[self.box_type[p, b] - 1]
                    captured = captured and next_to_agent if self.args.prevent_cannibalism else captured
                    # If the box is captured, remove it from the grid and terminate episode if specified
                    if captured:
                        # kill box
                        self.box_alive[p, b] = 0
                        self.grid[b, self.box[p, b, 0], self.box[p, b, 1], self.box_type[p, b]] = 0
                        # terminate if capture_terminal=True
                        terminated[b] = terminated[b] or self.capture_terminal
                        # determine reward for capture
                        rew = 0
                        rew += self.capture_stag_reward if self.box_type[p, b] == 1 else 0
                        rew += self.capture_hare_reward if self.box_type[p, b] == 2 else 0
                            ...
                            for a in range(self.n_agents):
                                ...
                                if self.agents_not_frozen[a, b] > 0 \
                                    ...
                                    for u in range(self.n_actions - 1):
                                        ...
                                        # ... to see if it would have moved onto box p's position ...
                                        if x[0] == self.box[p, b, 0] and x[1] == self.box[p, b, 1]:
                        ...
                        if self.print_push_box:
                            print("Captured %s at time %u, height %d and pos %u." %
                                  (("stag" if self.box_type[p, b] == 1 else "hare"), self.steps,
                                   self.env_max[0] - 1 - self.box[p, b, 0], self.box[p, b, 1]),
                                  "   Agents: ", self.agents_not_frozen[:, b],
                                  "   reward %g" % reward[b]
                                  )
                    else:
                        # If not, check if the box can rest and if so determine randomly whether it wants to
                        rest = (self.grid[b, self.box[p, b, 0], self.box[p, b, 1], 0] == 0) \
                               and (np.random.rand() < (self.p_stags_rest if self.box_type[p, b] == 1
                                                        else self.p_hare_rest)) \
                               or len(possible) == 0
                        # If the box decides not to rest, choose a movement action randomly
                        if not rest:
                            ...
                            if not (np.random.rand() < self.mountain_slope
                                    and self.grid[b, self.box[p, b, 0], self.box[p, b, 1], 0] == 0
                                    and (self.box_type[p, b] == 2 and u == 3 or self.box_type[p, b] == 1 and u == 1)):
                                # Execute movement
                                self.box[p, b, :], _ = self._move_actor(self.box[p, b, :], u, b,
                                                                         np.asarray([0, 1, 2], dtype=int_type),
                                                                         self.box_type[p, b])
            # Terminate batch if all box are caught or all agents are frozen
            terminated[b] = terminated[b] or sum(self.box_alive[:, b]) == 0 or sum(self.agents_not_frozen[:, b]) == 0
        ...
        if terminated[0] and self.print_push_box:
    ...
    def get_state_as_graph(self):
        ...
        # Enqueue all stags
        for s in range(self.n_heavy):
            state.append({'type': 'stag', 'pos': self.box[s, 0]})
        # Enqueue all hares
        for h in range(self.n_light):
            state.append({'type': 'hare', 'pos': self.box[self.n_heavy + h, 0]})
    ...
    def get_avail_agent_actions(self, agent_id):
        ...
        # If the agent is not frozen, the 'catch' action is only available next to a box
        if self.capture_action and self.agents_not_frozen[agent_id] > 0:
            avail_actions[self.action_labels['catch']] = 0
            # Check with virtual move actions if there is a box next to the agent
...
# ---------- PRIVATE METHODS ---------------------------------------------------------------------------------------
    ...
    def print_agents(self, batch=0):
        ...
        for p in range(self.n_boxes):
            if self.box_alive[p]:
                obs[self.box[p, batch, 0], self.box[p, batch, 1]] = -p - 1
        print(obs)
    ...
    def _observe(self, agent_ids):
        ...
        for b in range(self.batch_size):
            if all([self._is_visible(self.agents[agent_ids, b, :], self.agents[agent_ids[a], b, :])
                ...
                self._intersect_targets(grid, agent_ids, targets=self.box[:self.n_heavy, :, :], batch=b, target_id=1,
                                        targets_alive=self.box_alive[:self.n_heavy, :], offset=ashape)
                # Every agent sees intersected hares
                self._intersect_targets(grid, agent_ids, targets=self.box[self.n_heavy:, :, :], batch=b, target_id=2,
                                        targets_alive=self.box_alive[self.n_heavy:, :], offset=ashape)
            else:
                noinformation = True
    ...
    def _get_obs_intersection_old(self, agent_ids):
        ...
        for b in range(self.batch_size):
            if all([self._is_visible(self.agents[agent_ids, b, :], self.agents[agent_ids[a], b, :])
                ...
                # Every agent sees other intersected agents
                self._intersect_targets(grid, agent_ids, targets=self.agents, batch=b, target_id=0)
                # Every agent sees intersected box
                self._intersect_targets(grid, agent_ids, targets=self.box, batch=b, target_id=1,
                                        targets_alive=self.box_alive)
